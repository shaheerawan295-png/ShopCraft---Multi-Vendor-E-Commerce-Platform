...
import { Plus, Calendar, Percent, Trash2, ShieldCheck } from "lucide-react";
...
export default function AdminCouponsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        alert("Global Admin Coupon Created Successfully!");
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-8">
        <div>
          <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
            <ShieldCheck className="w-6 h-6 text-slate-800" /> Admin Global Coupons
          </h1>
          <p className="text-xs text-slate-500 font-medium">
            Manage global site-wide coupons and view all vendor coupons
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            >
              {loading ? "Creating..." : "Save Global Coupon"}
            </button>
          </form>

          <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
            <h2 className="text-base font-extrabold text-slate-900">All Active Coupons (Site-Wide & Vendor)</h2>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full ${
                              c.vendor ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"
                            }`}
                          >
                            {c.vendor ? "Vendor Coupon" : "Admin Global"}
                          </span>
                          <span
                            className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full ${
                              isExpired
